...
**dnf**是centos的包管理工具,vi /etc/yum.repos.d/CentOS-AF.repo配置好源之后， 可以直接dnf install 
...
rpm -qf /sfos/system/resource/lang/alarmd/alarmd.ini可以查安装包



**lsof**可以看文件被谁引用，由于unix一切皆文件，所以也可以看端口被谁监听

lsof -n |grep 80

**netstat** -anp也可以看端口监听情况

**ps** -auxf |grep dap可以看进程详细信息包括谁是谁的子进程等
